...
Not all of these mechanisms are helpful to fix script state after view transitions. Some are not related to navigation like async callbacks triggered by `setTimeout()`. Others can not really help due to limited DOM access, like for example the service workers.

In the following sections we will look at script tags and event listeners.
...
### Using Astro's Lifecycle Events
...
During view transitions, soft loads will ignore scripts executed since the last full page load. A full page load will wipe out Astro's memories of what it saw before. Therefore Astro executes scripts only once on first sight during view transitions. This is true for Astro versions >= 5.4.2.

Astro before 5.4.2 could potentially execute scripts multiple times during soft loads. A save approach for older versions of Astro is to use _external module script_ as the browser is guaranteed to only execute them once and [keeps _external module scripts_ until the next full page load](#external-module-scripts). is what you get automatically if [Astro processes a script tag with no further attributes](#astros-script-processing).  The browser only loads I.e. they do not get removed or updated  while you navigate using view transitions.
...
```
###	 Set Listeners on Persistent Elements
...
##### Built-in Swap
With the [built-in `swap()`](/jotter/astro/loader-swap/#swap-1), persistent positions for listeners to survive a soft load are:
- `window`
- `window.document`
...
Be aware that Astro processes the script tags. That might be a bit confusing if you are not yet used to it. Also be aware that script processing changed with Astro 5.0. The following table describes the behavior for Astro before 5.0.
...
:::note
What changed with Astro 5.0 is that small scripts automatically get inlined. This is even true for scripts marked as `external module scripts` in the [table above](#astros-script-processing). You can control this behavior by defining `vite.build.assetsInlineLimit` in your `astro.config.mjs` file, but that also affects the inlining of images and other resources.
:::

### Script Behavior on Initial Load

> Thanks to arsh,	 who has been a knowledgeable teacher: [older but still good](https://v8.dev/features/modules#defer)
...
External module scripts are special. These are scripts that have the form `<script type="module" src="..."/>` in the HTML document. As all other scripts, they are dealt with as soon as they are parsed. But the browser's module loader keeps track of the resources it has loaded *since the last full page load*. If a module with the same `src` attribute value was already seen before, it would not be loaded and executed again. This is a browser / JavaScript feature and it is completely independent of the Astro version you use.  You can not even override this behavior using [`data-astro-rerun`](#data-astro-rerun)
As a consequence, external module scripts will not be re-executed during soft loads.

### Behavior with View Transitions

On navigation to another page using Astro's `<ClientRouter />` component, loading and execution of scripts is handled differently compared to normal full page loads. Due to the soft load approach, view transitions do only change the current DOM to resemble the one for the new page.

Astro might execute all scripts that it finds on the target page. But there are exemptions given by these two rules:
1. **External module scripts** for which the value of the `src` attribute is already known to the browser's module loader **are not executed again**. True for all versions of Astro, see the [section above](#external-module-scripts).
2. **Astro excludes a script** from execution if the same script **was already executed** since the last full page load (**before Astro 5.4.2:** if the script was found **on the previous page**). Two external scripts are considered to be the same if they have the same `src` attribute value. Two inline scripts are considered the same if they have the same embedded code, character for character.
...
#### data-astro-rerun
You can opt-out of the second rule in the [list above](#behavior-with-view-transitions) by setting the `data-astro-rerun` attribute for the script. This automatically turns the script into an inline script. Therefore, you cannot use TypeScript here. But you can use imports if you add `type="module"`.
