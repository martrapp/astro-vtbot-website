...
:::note
While the pattern with `astro:page-load` only works if `<ViewTransitions />` are enabled, the pattern using `astro:after-swap` works fine with and without `<ViewTransitions />`.
:::
...
On navigation to another page using Astro's `<ViewTransitions />` component, loading and execution of scripts is handled differently. Due to the soft load approach, view transitions do not do a full page load, but they change the current page to resemble the new one.
