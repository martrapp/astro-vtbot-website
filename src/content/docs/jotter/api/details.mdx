...
```ts title="dom-view-transition.d.ts (Level 2)" {5,11,21} "As of level 1 plus the following" "StartViewTransitionOptions"
...
// there are also two new events that are raised during cross-document navigation:
...
```css title="use view-transition-class"
