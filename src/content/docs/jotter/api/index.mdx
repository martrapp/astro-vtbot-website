...
The API introduces a new CSS property called `view-transition-name` that can be used to mark DOM elements to be animated. It also provides a single function called `startViewTransition()`, which can be found in the `window.document` object. You call this function when you want to start the visual transition and you provide an _update callback_ that can convert the current document to its future version.

This is how you call `window.document.startViewTransition()`. The result contains some promises and a function to cancel the visual transition. For [details see the API description](/jotter/api/processing/).
...
[^1]: plus some other CSS properties, see https://drafts.csswg.org/css-view-transitions/#style-transition-pseudo-elements-algorithm
...
The `<html>` root element of the document is an exception to this rule, as it gets special treatment by the View Transition API: here the position is always (0,0) and the size is set to that of the viewport, which makes morph transitions for `:root` effectively a no-op.
