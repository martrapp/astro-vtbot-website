...
<dd><p>Make sure that you have added the event listener to the <code>document</code> object and not to the <code>window</code> object.</p><p>Check the spelling of the event name.</p>
...
```
:::caution
Starting with Astro 6, these functions are deprecated. In Astro 7, they will be removed.

To prepare for the removal, use the event parameter without an explicit type and let typescript infer `TransitionBeforePreparationEvent` or `TransitionBeforeSwapEvent`.
:::