...
```
The code adds the `falseStart` type to the view transitions set of types if the old and new page URLs match. It might seem a bit complicated, but this approach also works in Safari, which in older versions lacks support for the Navigation API. The condition is determined on the old page and passed to the new page via a session storage entry.
