...
For multi-page applications, you really want to use native cross-document view transitions. With a bit of luck, cross-document view transitions will make it into Interop 2026. And of course, that does not necessarily mean it will take until the end of the year. Safari, for example, took only a few months to follow same-document view transitions with the cross-document version.
...
Cross-document view transitions are not implemented in Firefox yet. But there are also restrictions for same-document view transitions as Level 1 does not support all features of same-document view transitions. Some newer features are defined in Level 2 of the spec and some of them are not yet implemented in Safari, not even in Edge and of course not yet in Firefox.   

#### View Transition Types

Firefox does not yet support the new signature where you can pass an object that has optional members for the update callback and for an array of view transition types, because view transition types are defined in Level 2 of the View Transition API. Consequently, there is no `types` property in the ViewTransition object and the `:active-view-transition-type()` pseudo-classes are also not supported yet.

:::tip
If you want to focus on view transition features that work reliably across all browsers, even in older Chromium and Safari versions, consider ignoring view transition types. They may look fancy, and you may think you should use them because someone specifically invented them. But in reality, you can easily achieve the same result for same-document view transitions yourself.

Instead of using a view transition type like `my-type`, use a CSS class such as `vtt-my-type` on the document root. And instead of writing `:active-view-transition-type(my-type)`, simply use `:root.vtt-my-type`. It works exactly the same and even takes less typing. The `vtt-` prefix helps prevent conflicts with other CSS classes.

All that is left is adding and removing the class. Add it right before calling `startViewTransition()` and remove it automatically when the view transition ends:
```js title="Definition"
	const prefixed = types.map((t) => 'vtt-' + t);
	const classes = document.documentElement.classList;
	classes.add(...prefixed);
	let transition = document.startViewTransition(() => {...});
	transition.finished.finally(() => classes.remove(...prefixed));
```
```css title="Usage"
:root.vtt-test {
	.card {
...
```
This pattern will even work in the future with scoped view transitions, where you replace `:root` with the element on which you call `startViewTransition()`.

If you are interested in an automatic substitute for view transition types that wraps all of the above, see [the polyfill](https://vtbag.dev/tools/utensil-drawer/#usetypespolyfill-always--auto--never) and [the PostCSS plugin](https://vtbag.dev/tools/utensil-drawer/#the-postcss-active-view-transition-type-postcss-plugin) from vtbag.dev.
:::
