...
|10|[Source Element](#source-element)|Know which element triggered the transition|Have to use the Navigation API instead|
...
But don't worry, the Bag has your back with the [direction script](https://vtbag.dev/tools/turn-signal/#directions-script) that can work as a drop in replacement!

A future version of the web platform will cover directional navigation with the history check of the `@navigation` at-rule:
```
...
```
But it will take a bit until this gets _Baseline Widely Available_ status.
...
Now that the Navigation API has _Baseline Newly Available_ status, you can use the `sourceElement` property of the NavigationEvent instead.
