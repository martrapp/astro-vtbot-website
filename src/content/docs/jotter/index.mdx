...
```
:::note
With version 5.0, Astro's `<ViewTransition />` component was renamed to `<ClientRouter />`. `<ViewTransitions />` is still supported but deprecated. This Jotter still uses the old `<ViewTransitions />` terminology until it is migrated to Astro 5.0 Starlight.
:::
