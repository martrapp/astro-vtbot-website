---
title: "SPA-like Look & Feel: Add View Transitions to Starlight"
---
...
Without any parameters it selects all headings on all pages and assigns the view transition names `vtbot-hx-0`, `vtbot-hx-1`, &hellip;. This has the effect that the n-th heading on the current page and the n-th heading on the next page from a [view-transition-group](/jotter/api/#morph-animations). The headings are cut out of the page context and the old heading morphs into the new heading when navigating.
...
You can have the content of the sidebar replaced on each navigation. To do this, add the `replaceSidebarContent` attribute to the `VtbotStarlight` tag in your [`Head` component](/jotter/starlight/guide/#step-2-create-a-head-component).
```astro title="/src/components/starlight/Head.astro"
...
If you want to take control over the current page marker yourself, you can switch of the default update by adding the `retainCurrentPageMarker` attribute to the `VtbotStarlight` tag in your [`Head` component](/jotter/starlight/guide/#step-2-create-a-head-component). This will also disable auto-opening and auto-scrolling to the current page marker.
...
<Code code={DemoHead3} lang="astro" title="./src/components/starlight/Head.astro" ins={[5,6,7,8,16,17,19,20,22,23,25,26,27]} />
...
* `starlight-util` (f.k.a. `starlight-muti-sidebar`)

I plan to support compatibility to both also in the future.
