...
As of end of October 2024, my new [Starlight site about browser-native cross-document view transitions](https://vtbag.pages.dev) (i.e. View Transitions without the `<ViewTransitions />` component) is completely running on &hellip; you guess it &hellip; **browser-native cross-document view transitions**!

As I did for the current site before, I will share the necessary knowledge and components over there during the next months.
...
If you're satisfied with having the view transition effect work only in Chromium-based browsers (such as Chrome, Edge, and Opera) and can do without it in Firefox, and other browsers that don't yet support the cross-document features of the View Transition API, you can enable browser view transitions by simply adding the following line to your [custom.css](https://starlight.astro.build/guides/css-and-tailwind/#custom-css-styles) and you are all set.
...
Otherwise stay tuned for how to add Astro's `<ViewTransitions>` support to Starlight and enjoy view transitions **on all browsers** including Safari and thanks to Astro's simulation even on Firefox and other browsers that do not yet have native support for the View Transition API.
