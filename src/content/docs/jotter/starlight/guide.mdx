...
Below this line, you will find information on how to add view transitions using Astro's `<ClientRouter>` component. I do **not recommend this approach anymore in 2025**, but it might still suit you if you have a need to share state across your pages.


In this case, stay tuned for how to add Astro's `<ClientRouter>` support to Starlight and enjoy view transitions **on all browsers** and thanks to Astro's simulation even on Firefox and other browsers that do not yet have native support for the View Transition API.
