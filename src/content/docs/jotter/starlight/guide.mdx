...
Otherwise stay tuned for how to add Astro's `<ViewTransitions>` support to Starlight and enjoy view transitions **on all browsers** and thanks to Astro's simulation even on Firefox and other browsers that do not yet have native support for the View Transition API.
