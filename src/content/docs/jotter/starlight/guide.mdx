---
title: "Guide: <ClientRouter /> View Transitions for Starlight"
description: How to turn your Starlight site in to a SPA and enable view transitions with Astro's <ClientRouter />.
---

import BoT from "/src/components/BoT.astro"
import AnimateImages from "/src/components/AnimateImages.astro"

If you want to get rid of visible full page loads in your Starlight site [you have different options](/jotter/starlight/). This guide describes how this can be done using Astro's `<ClientRouter />`.

Stay tuned for how to add Astro's `<ClientRouter>` support to Starlight and enjoy view transitions **on all browsers** and thanks to Astro's simulation even on Firefox and other browsers that do not yet have native support for browser-native cross-document view transitions.
...
```
<details>
<summary>This is for Starlight versions < 0.32</summary>
```astro title="./src/components/starlight/Head.astro"
---
...
You can add a `transition:animate=...` [directive](https://docs.astro.build/en/guides/view-transitions/#built-in-animation-directives) to the `<VtbotStarlight>` component. The Bag will teleport the directive from there to the `<main>` element of the Starlight page.  This is the way to enable view transition animations for the main content area of a Starlight site for all browsers.[^add-props]

```astro title="How to add animations to the main area"
<VtbotStarlight transition:animate="fade">
	<StarlightHead><slot /></StarlightHead>
</VtbotStarlight>
```
[^add-props]: Add `{...Astro.props}` to the components if you are using a Starlight version < 0.32.
...
```astro title="./src/components/starlight/Head.astro"
---
// For Starlight version < 0.32 add Astro.props as shown in Step 2 above 
import StarlightHead from '@astrojs/starlight/components/Head.astro';
import VtbotStarlight from 'astro-vtbot/components/starlight/Base.astro';
...
:::note
 For Starlight version < 0.32, add `{...Astro.props}` to the components.
:::
