...
:::caution
Currently, view transitions work for the `doc` template, but the sidebar is not displayed correctly  when navigating from or to pages with `template: splash`. This is a known issue and will be fixed soon.
:::
