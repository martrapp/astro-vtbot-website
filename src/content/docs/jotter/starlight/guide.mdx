...
Starlight supports a lot of different config options. The Bags view transition support is so far only tested with a few typical combinations. A first observation with multi-language site is that selecting another language forces a full page reload instead of a view transition.
