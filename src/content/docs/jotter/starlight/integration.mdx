...
Visit the [corresponding section in the introduction](/jotter/starlight/#hook-into-starlight) to see where we are in  the big picture of The Bag's Starlight support.
...
Once the <BoT/> provides more components for Starlight, this file is also a good place to add them to enable more functionality for the Starlight website.
...
- It includes `<ReplacementSwap>` to [preserve Starlight's app state](/jotter/starlight/app-state/)
- If an transition scope was defined for the main section, it inserts its name to the page for later reference, see [Defining Animations](/jotter/starlight/transitions/)
...
2. You find here calls for all the features mentioned in the [mandatory actions](/jotter/starlight/#mandatory-actions) section.

3. If you do not only want to know _what_ happens but also _how_, I refer you to the corresponding sections:
- [Managing the App State](/jotter/starlight/app-state/) for `markMainFrameForReplacementSwap`
- [Updating the Sidebar](/jotter/starlight/sidebar/) for `updateSidebar` and `closeMobileMenu`
- [Defining Transitions](/jotter/starlight/transitions/) for `setMainTransitionScope`
