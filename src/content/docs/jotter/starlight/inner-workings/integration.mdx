...
The way to enable view transitions on an Astro site is to add Astro's `<ViewTransitions />` component into the `<head>` of every page. Starlight has a dedicated mechanism to insert elements into the `<head>`. But that works for HTML elements, only. It can't be used to insert an Astro component.
