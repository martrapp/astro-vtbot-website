...
As you know from the section on [updating script state](/jotter/astro/scripts/) adding the `<ViewTransitions />` component is the simpler part. Ensuring that script state is  up to date might be more of a challenge. From what you saw there, it is best practice to use the lifecycle events to update the state of your scripts.
