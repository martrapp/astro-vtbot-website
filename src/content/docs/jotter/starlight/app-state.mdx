...
Visit the [corresponding section in the introduction](/jotter/starlight/#keep-app-state-up-to-date) to see where we are in  the big picture of The Bag's Starlight support.
...
![astro.build docs](../../../../assets/starlight.astro.build_getting-started_dark.png)
</div><div class="dark:sl-hidden">
![astro.build docs](../../../../assets/starlight.astro.build_getting-started_light.png)
</div>
...
## Keeping the Frame Part of the DOM ...
 ...
 [During `swap()`](/jotter/astro/loader-swap/#swap) we would simply copy the application frame to the next page. Doing this with `transition:persist` requires a deep knowledge of the structure of a Starlight page and is vulnerable to changes in that structure.
...
```
See this [line in its context](/jotter/starlight/integration/#structure-of-the-base-component) in `astro-vtbot/components/starlight/Base.astro.
...
```
**Revealed:** This reveals what [Lines 12 & 13](/jotter/starlight/integration/#connecting-to-starlight) of the StarlightConnector do.
...
> **Spoiler**: *The first assumptions does not hold* when we navigate to pages with a different layout, especially the `splash` layout. Read more about the consequences [on the next page](/jotter/starlight/sidebar/)!
