...
The Starlight Plugin provides an easy way to define [expect links](https://vtbag.dev/fwvt/twinkling/#await-page-content) using a frontmatter filed called `renderBlocking`. If you do not use this option, you can alternatively use Starlight's [head fields](#how-to-expect-content-without-the-plugin) to define such links.
...
To enable this option, import and include the plugin that adds the end-of-markdown id. For Starlight 0.32 - 0.40, use the `remarkEndOfMarkdown` plugin. For Sätteri (Starlight 0.41+), use the `hastMarkEndOfMarkdown` plugin. 


##### For Sätteri (Starlight 0.41+)
```ts title="astro.config.mjs" {3,8}
import { defineConfig } from 'astro/config';
import { satteri } from '@astrojs/markdown-satteri';
import { hastMarkEndOfMarkdown, viewTransitions } from "astro-vtbot/starlight-view-transitions";
...
##### For Starlight 0.41+ w/o Sätteri
If you happen to run Starlight 0.41+ without Sätteri, you probably see a warning that nudges you to use the unified processor. Here you go:
...
If you want to replace the default id `end-of-markdown` with a custom id, both plugins accept a string parameter for the id name. For example, to use `below-the-fold` instead of `end-of-markdown`, you would do this:

```ts title="astro.config.mjs"
...
	hastPlugins: [hastMarkEndOfMarkdown("below-the-fold")],
...
```	
