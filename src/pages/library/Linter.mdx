...
## Usage
[Install astro-vtbot](../Installation/) in your project as an Astro integration with `npx astro add astro-vtbot` or as a node package with `npm install astro-vtbot`. If you choose the integration option, `vtbot` will automatically add the `<Linter/>` component to all the pages that use `<ViewTransitions>`.
