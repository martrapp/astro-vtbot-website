...
Place the `<BrakePad/>` component in the `<head>` of a page near Astro's `<ClientRouter />`. Now when you leave that page using a view transition, the page will simulate a slow loading time of 2000ms. By default, the delay is only active in DEV mode and not if you build for production.
...
|------------|------|--------|
| duration | number | The duration of the delay in milliseconds. The default is 2000 ms.|
