...
Animations where the name beginns with `-ua ` are browser defined. Those might be static like `-ua-view-transition-fade-in` or even denamically generated depending on your page like `-ua-view-transition-group-anim-astro-izu6424p-1` in the example above.
...
The `<VtBotDebug>` component offers a boolean property to control its behaviour:
