...
This example uses view-transition-class properties to address images of the `<main>` elements of all three pages at once. See the the[@vtbag](https://vtbag.dev/tools/turn-signal/#configuration-and-usage) pages for more examples and on how to use  `:active-view-transition-type(<type>)` for sites that do not need the compatibility to `transition:animate`.
