...
1. The scripts of the new DOM are scanned. Scripts that already ran since the last full reload are marked to prevent execution after `astro:after-swap`.
2. The attributes of the `<html>` element are changed to mirror those of the new DOM.
3. Direct children of the `<head>` that are not present in the new DOM are removed, `<head>` children that are only present in the new DOM are added. `<head>` children that are in both DOMs remain untouched. 
