...
<dd><p>Yes. Alternatively, you can define the `view-transition-name` CSS property in a style sheet or style attribute.</p></dd>
...
```
### How exactly is the DOM changed when I navigate to the next page?

These are implementation details, not part of the API and might change without prior notice, but if you are interested in the details:

This is how the default swap method works. As parameter it gets the new DOM and it alters the current DOM in place.

1. The scripts of the new DOM are scanned. Scripts that are already present on the current page  are marked to prevent re-execution after `astro:after-swap`.
2. The attributes of the `<html>` element are changed to mirror those of the new DOM.
3. Direct children of the `<head>` are removed and those present in the new DOM are moved in.  Some children of the current DOM are skipped and remain untouched, i.e. they are not deleted and their matching counterpart is not moved into the current DOM:
   - Stylesheet links with same href that are already present in both DOMs
   - Elements with same `data-astro-transition-persist` name in both DOMS
4. Focus and selection are saved
5. The body is swapped
6. Focus and selection are restored

Body swapping is done in two steps:
* The `<body>` element of the current DOM is replaced with the `<body>` element of the new DOM.
* Elements that are marked with the same `data-astro-transition-persist` name in both DOMs' bodies are replaced in the now current body with the corresponding elements from the old body. Elements with the same `data-astro-transition-persist` name can have different element types, attributes and children.

If you use `astro-vtbot`'s [&lt;ReplacementSwap/> component](/library/ReplacementSwap/), the body is only altered for elements that are marked with `data-vtbot-replace`, the others are left untouched. `data-astro-transition-persist` attributes are ignored.

For attributes of the `<html>` element and child elements of the `<head>` element, their order might change  during swap. But if they are part of the current and the new DOM they are not moved/added/removed but just left unchanged. The `<html>` and `<head>` elements themselves are never removed or added.

Event listeners on `window` and `window.document` are retained. References to elements of the old DOM that didn't make it to the current DOM are invalid after swap. Event listeners for elements that are no longer part of the current DOM after the swap are never called and newly added elements have no listeners unless they have been explicitly set in the `astro:after-swap` or `astro:page-load` handlers.

Scripts found in the new DOM will be executed between `astro:after-swap` and `astro:page-load` but only if they hadn't been marked as "known and already ran" in step 1 above.
