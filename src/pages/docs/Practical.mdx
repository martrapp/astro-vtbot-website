...
```
### How exectly is the DOM changed when I navigate to the next page?

These are implementation details, not part of the API and might change without prio notice, but if you are interested in the details:
...
3. Direct children of the `<head>` are removed and those present in the new DOM are moved in.  Some children of the current DOM are skipped and remain untouched, i.e. they are not deleted and their matching counterpart is not moved into the current DOM: 
  a. Stylesheet links with same href that are already present in both DOMs
	b. Elements with same `data-astro-transition-persist` name in both DOMS
4. Focus and selection are saved
5. The body is swapped
6. Focus and selection are restored

Body swapping is done in two steps:
1. The `<body>` element of the current DOM is replaced with the `<body>` element of the new DOM. 
2. Elemets that are marked with the same `data-astro-transition-persist` name in both DOMs' bodies are replaced in the now current body with the corresponding elements from the old body. Elements with the same `data-astro-transition-persist` name can have different element types, attributes and children.
