...
This demo uses the <a href="/library/Portal/" data-astro-reload>`<Portal/>` component</a>  to intercept all navigation with a portal page. The navigation is automatically resumed when the content of the target page is available. This way it can be used to show a loading animation. On the transition from page A to B with the intermediate portal page C, there are two view transitions: One from A to C and one from C to B.
